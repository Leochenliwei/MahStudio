...
1. **端口占用**：确保8001端口未被占用，否则需要修改.env文件中的PORT配置
...
默认情况下，前端会连接到 `http://localhost:8001/api`，无需额外配置。
...
1. **获取你的内网IP地址**：
   ...
   ```
   输出类似：`inet 10.0.104.207 netmask 0xfffffe00 broadcast 10.0.105.255`

2. **修改前端API地址**：
   编辑 `.env` 文件，将 `VITE_API_HOST` 修改为你的内网IP地址：
   ```env
   VITE_API_HOST=10.0.104.207
   ```
...
4. **其他同事访问**：
   其他内网同事可以通过以下地址访问：
   - 前端：`http://你的内网IP:5173`
   - API：`http://你的内网IP:8001/api`

### 端口配置

所有端口配置都集中在 `.env` 文件中：
- `PORT`：后端服务器端口
- `VITE_API_PORT`：前端API端口（应与PORT保持一致）
- `VITE_API_HOST`：前端API主机地址
